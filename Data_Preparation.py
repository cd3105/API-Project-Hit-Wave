...
new_top_200_billboard_df = pd.DataFrame({"Artist": unique_top_200_billboard_df["Artist"], "Song Title": unique_top_200_billboard_df["Song"].apply(lambda x: x.upper())})
...
all_unique_songs = pd.concat([new_top_40_df, new_top_100_billboard_df, new_top_200_billboard_df, new_hot_100_df, new_the_hot_100_df, new_weekly_hot_100_df, new_million_spotify_df]).sort_values(by="Artist").drop_duplicates(subset=["Song Title"]).reset_index(drop=True)
...
all_unique_songs_top_200_billboard = all_unique_songs.copy()
all_unique_songs_top_200_billboard["Hit"] = all_unique_songs_top_200_billboard["Song Title"].apply(lambda x: 1 if x in list(new_top_200_billboard_df['Song Title']) else 0)
all_unique_songs_top_200_billboard.to_csv('Preprocessed and Labeled Datasets/Labeled_Songs_per_Top_200_Billboard.csv', index=False)
...
print(f'Number of Songs in Top 200 Billboard: {len(new_top_200_billboard_df)}')
print(f'Number of Hit Songs in Dataset based on Top 200 Billboard: {sum(all_unique_songs_top_200_billboard["Hit"])}')
